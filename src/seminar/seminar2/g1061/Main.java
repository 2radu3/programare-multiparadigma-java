...
*/
            ...
            System.out.println("\nLista sortata dpa valoare: ");
            for(MijlocFix mijloaceFixe:lista) {
                System.out.println(mijloaceFixe);
            }

            //selectie dupa numar inventar
            long nrInventar = 3L;
            int k = lista.indexOf(new MijlocFix(nrInventar)); //foloseste equals din elemnt patrimonial
            if(k==-1) {
            ...
            mijlocFix.setValoare(50000); //sau valoare
            Collections.sort(lista,comparatorValoare);
            k = Collections.binarySearch(lista,mijlocFix,comparatorValoare);
            if (k >= 0) {
                System.out.println("\nMijloc fix cautat: \n" + lista.get(k));
            } else {
                System.out.println("\nNu exista mijloc fix cu valoare "+mijlocFix.getValoare());
    ...
    private static void printare(String numeFisier) {
        try(PrintWriter fout = new PrintWriter(numeFisier)) { //in try se face automat close pt fisier
            for(MijlocFix mijlocFix:lista) {
    ...
    private static void restaurare(String numeFisier) {
        ...
        ObjectInputStream fi = new ObjectInputStream(in)) {
            lista.clear(); //clear sterge obiectele / elibereaza lista
            while(in.available()!=0) {
                lista.add((MijlocFix)fi.readObject());
            }
        }
        catch (Exception ex) {
            System.out.println(ex);
